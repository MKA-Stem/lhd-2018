...
class Game {
  ...
  _updateLeaderboard() {
    const players = this.players.map(e => ({id: e.id, name: e.name, score: e.score}));
  ...
  _enter_judging() {
    ...
    this.room.emit('choices', {
      choices: this.players.map(p => ({id: p.id, name: p.name, choice: p.choice}))
    });
  }
  ...
  _cl_judgement(player, card) {
    ...
    const winner = this.players.filter(e => e.choice && e.choice.id === card.id)[0];
